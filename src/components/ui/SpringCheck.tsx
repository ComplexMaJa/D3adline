...
const RULE_END = 0.84;
const SWELL = 0.35;
const TICK_PATH = "M5 13.5L9.5 18L19 6.5";
...
const SpringCheck: React.FC<SpringCheckProps> = ({
  ...
  const cssVars = {
    ...
    '--sc-ring': `${ring}px`,
    '--sc-gap': `${gap}px`,
    '--sc-row': minHeight !== undefined ? (typeof minHeight === 'number' ? `${minHeight}px` : minHeight) : `${Math.max(44, boxSize + 16)}px`,
    ...
    >
      ...
      <span className="relative flex-1 min-w-0 inline-flex items-center">
        <span className="relative inline-block max-w-full">
          <span ref={wordRef} className="inline-block break-words" style={{ opacity: r.word }}>
            {label}
          </span>
          {strike !== 'none' ? (
            <span
              ref={ruleRef}
              className="pointer-events-none absolute inset-x-0 top-[50%] -translate-y-1/2 h-[var(--sc-rule)] rounded-[2px] bg-current [transform-origin:var(--sc-origin)]"
              aria-hidden="true"
